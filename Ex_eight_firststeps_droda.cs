    ...
    public class FirstSteps : RenderCanvas
    {
        ...
        public override void Init()
        {
           ...
           RC.ClearColor = new float4(0.7f, 1.0f, 0.5f, 1.0f);
            ...
            for (int i = 0; i < 5; i++)
            {
                var cubeTransform = new TransformComponent { Scale = new float3(1, 1, 1), Translation = new float3((i-3)*10, 0, 0) };
      ...
      { 
        ...
        public override void RenderAFrame()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                _cubes[i].GetTransform().Translation = new float3((i - 3) * 10, 5 * M.Sin((3 * TimeSinceStart)+ i ), 0);
                _cubes[i].GetTransform().Rotation = new float3(0, 6 * TimeSinceStart, 0);
                _cubes[i].GetTransform().Scale = new float3(0.5f *M.Sin(TimeSinceStart*3),0.5f *M.Sin(TimeSinceStart*3),0.5f *M.Sin(TimeSinceStart*3));
